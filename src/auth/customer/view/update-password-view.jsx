...
import { useChangePasswordForForgotMutation } from 'src/redux/slices/customer/auth';
...
export function SplitUpdatePasswordView() {
  ...
  useEffect(() => {
    if (!localStorage.getItem('customerForgotToken')) {
      router.push(paths.customerAuth.resetPassword);
    }
  }, []);
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      changePassword({ otp: data?.code, newPassword: data?.password });
    } catch (error) {
      console.error(error);
    }
  });
