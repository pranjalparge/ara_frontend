...
export const SKILLS = [...Array(3)].map((_, index) => ({
  ...
  value: [90, 89, 98][index],
}));

// ----------------------------------------------------------------------

export function AboutWhat({ sx, ...other }) {
  const [getWhyus, { data, isLoading, isSuccess }] = useGetStaticMutation();
  useEffect(() => {
    getWhyus();
  }, []);
  return (
              ...
              <Grid xs={8}>
                <m.div variants={varFade().inUp}>
                  <Image
                    ...
                    src={`${CONFIG.assetsDir}/assets/images/about/kitchensetup.jpeg`}
                    ratio="3/4"
                    sx={(theme) => ({
                      borderRadius: 3,
                      boxShadow: `-40px 40px 80px ${varAlpha(theme.vars.palette.grey['500Channel'], 0.24)}`,
